...
export const DataForm = ({ onBack }: DataFormProps) => {
  ...
  const loadDataOperators = async () => {
    try {
      ...
      console.error('Failed to load data operators:', error);
      toast({
        title: "Error",
        description: "Failed to load data providers. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const processDataService = async (reference: string, amountNum: number) => {
    try {
      ...
      toast({
        title: "Data Bundle Sent Successfully!",
        description: `₦${amountNum.toLocaleString()} data bundle sent to ${phoneNumber}`,
        duration: 5000,
      });
  ...
  const handleDirectPayment = async (amountNum: number) => {
    try {
      console.log('Initializing direct payment for data...');
